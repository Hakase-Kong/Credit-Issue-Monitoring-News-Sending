...
def render_important_article_review_and_download():
    ...
    if not st.session_state.get("important_articles_preview"):
        st.info("아직 중요 기사 후보가 없습니다. 먼저 자동 생성을 실행하세요.")
        return

    # 🎯 중요 기사 목록 UI - 박스 형태로 렌더링
    ...
    with col_action2:
        if st.button("🔁 선택한 기사 교체 (왼쪽에서 1개 선택 필요)"):
            ...
            if len(left_selected_keys) != 1 or len(right_selected_indexes) != 1:
                st.warning("왼쪽에서 1개, 오른쪽에서 1개만 선택해주세요.")
                return

            from_key = left_selected_keys[0]
            m = re.match(r"^[^_]+_[0-9]+_(.+)$", from_key)
            if not m:
                st.warning("선택 key 파싱 오류")
                return

            uid = m.group(1)
            selected_article = None
            for kw, arr in st.session_state["search_results"].items():
                for art in arr:
                    if re.sub(r'\W+', '', art["link"])[-16:] == uid:
            ...
            if not sentiment:
                ...
                st.session_state[f"summary_{cleaned_id}"] = ("", "", sentiment, "")
