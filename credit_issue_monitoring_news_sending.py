...
EXCLUDE_TITLE_KEYWORDS = [
    "야구", "축구", "배구", "농구", "골프", "e스포츠", "올림픽", "월드컵", "K리그", "프로야구", "프로축구", "프로배구", "프로농구", "유도", "꿈나무", "올림픽", "탁구",
    "부고", "부음", "인사", "승진", "임명", "발령", "인사발령", "인사이동",
    "브랜드평판", "브랜드 평판", "브랜드 순위", "브랜드지수", "브랜드", "페스타", "페스티벌", "할인 행사", "서울거리", "폴리스라인", "상금", "경진대회"
    "코스피", "코스닥", "주가", "주식", "증시", "시세", "마감", "장중", "장마감", "거래량", "거래대금", "상한가", "하한가",
    "봉사", "후원", "기부", "우승", "무승부", "패배", "스포츠", "스폰서", "지속가능", "ESG", "위촉", "이벤트", "사전예약", "챔프전",
    "프로모션", "연극", "공연", "어르신", "링컨", "에비에이터", "NH퍼플통장", "골라담기", "태극기", "공채", "메디컬테스트", "V리그", "김연경", "레베카",
    ...
    "테니스", "집중호우", "수해 복구", "온열질환", "출시", "금융교육", "유튜브 구독자", "김장", "취약계층", "할인행사", "안성재", "에너지음료", "문화교류"
]
...
favorite_categories = {
    ...
    "5대금융지주": ["신한지주", "하나금융", "KB금융", "농협금융", "우리금융"],
    "5대시중은행": ["농협은행", "국민은행", "신한은행", "우리은행", "하나은행"],
    "카드사": ["KB국민카드", "현대카드", "신한카드", "BC카드", "삼성카드"],
    "캐피탈": ["한국캐피탈", "현대캐피탈"],
    "지주사": ["SK이노베이션", "GS에너지", "SK", "GS"],
    "에너지": ["SK가스", "GS칼텍스", "S-Oil", "SK에너지", "코리아에너지터미널"],
    "발전": ["GS파워", "GSEPS", "삼천리"],
...
excel_company_categories = {
    ...
    "에너지": [
        "SK가스", "GS칼텍스", "S-Oil", "SK에너지", "코리아에너지터미널"
    ],
    "발전": [
        "GS파워", "지에스이피에스", "삼천리"
    ],
...
common_filter_categories = {
    "신용/등급": [
        "신용등급", "등급전망", "하락", "강등", "하향", "상향", "디폴트", "부실", "부도", "미지급", "수요 미달", "미매각", "제도 개편", "EOD", "사고"
    ],
    "수요/공급": [
        "수요", "공급", "수급", "둔화", "위축", "성장", "급등", "급락", "상승", "하락", "부진", "심화", "매수", "매도"
    ],
    "실적/재무": [
        "실적", "매출", "영업이익", "적자", "손실", "비용", "부채비율", "이자보상배율", "이익", "수익", "흑자", "건전성", "충당금", "이익", "수익", "흑자", "건전성", "충당금"
    ],
    "자금/조달": [
    ...
    "지배구조/법": [
        "횡령", "배임", "공정거래", "오너리스크", "대주주", "지배구조", "유상증자", "자본확충", "위반", "불법", "경영권 분쟁"
    ]
}
                ...
                if st.button("🗑 선택 해제 (전체)"):
                    ...
                    for key, is_checked in st.session_state.article_checked.items():
                        if is_checked:
                            ...
                            idx = int(m.group(2))
                            uid_tail = m.group(3)
                            for art in results.get(keyword, []):
                                if re.sub(r"\W+", "", art["link"])[-16:] == uid_tail:
